      ...
      if [[ "$line" =~ / ]]; then
         echo "$line" | while read a1 rgb1 a2 rgb2 # a: altitude
                        do
                           ...
                           s1="$(echo "$hsv1" | cut -f2 -d' ' | awk -v scale="$scale" '{print $1/scale}')"
                           v1="$(echo "$hsv1" | cut -f3 -d' ' | awk -v scale="$scale" '{print 1 - (1 - $1)/scale}')"
                           r2="$(echo "$rgb2" | cut -f1 -d/)"
                           g2="$(echo "$rgb2" | cut -f2 -d/)"
                           b2="$(echo "$rgb2" | cut -f3 -d/)"
                           hsv2="$("$dir"/hsv_from_rgb.rb.sh "$r2" "$g2" "$b2")"
                           h2="$(echo "$hsv2" | cut -f1 -d' ')"
                           s2="$(echo "$hsv2" | cut -f2 -d' ' | awk -v scale="$scale" '{print $1/scale}')"
                           v2="$(echo "$hsv2" | cut -f3 -d' ' | awk -v scale="$scale" '{print 1 - (1 - $1)/scale}')"
